...
// src/environment.ts
var availEnvSchema = z.object({
  AVAIL_ADDRESS: z.string().min(1, "Avail address is required"),
  AVAIL_SEED: z.string().min(1, "Avail account seed phrase is required")
});
async function validateAvailConfig(runtime) {
  try {
    const config = {
      AVAIL_ADDRESS: runtime.getSetting("AVAIL_ADDRESS") || process.env.AVAIL_ADDRESS,
      AVAIL_SEED: runtime.getSetting("AVAIL_SEED") || process.env.AVAIL_SEED
    };
...
Tx Hash: ${txResult.txHash}, Block Hash: ${txResult.status.asFinalized}`
        ...
        if (callback) {
          callback({
            text: `Data submitted successfully! tx hash: ${txResult.txHash} Block Hash: ${txResult.status.asFinalized} `,
            content: {}
          });
        }
        return true;
...
import {
  getDecimals,
  initialize as initialize2,
  formatNumberToBalance,
  getKeyringFromSeed as getKeyringFromSeed2,
  isValidAddress
} from "avail-js-sdk";
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    if (content.amount != null && content.recipient != null) {
      try {
        ...
        const txResult = await new Promise(
          (res) => {
            api.tx.balances.transferKeepAlive(content.recipient, amount).signAndSend(
              keyring,
              options,
              (result) => {
                elizaLogger2.log(
...
Tx Hash: ${txResult.txHash}, Block Hash: ${txResult.status.asFinalized}`
        ...
        if (callback) {
          callback({
            text: `Transfer completed successfully! tx hash: ${txResult.txHash} Block Hash: ${txResult.status.asFinalized} `,
            content: {}
          });
        }
        return true;
